"""Object-based cell extraction and tracking (PDF Stage 2).

Uses scipy.ndimage.label to group contiguous reflectivity pixels into discrete
storm objects, calculates their centroids, areas, and intensities, and tracks
them between frames to compute individual growth and velocity.
"""
...
def cells_from_dbz(dbz, mask, pixel_to_latlon, bx, by, km_per_px):
    ...
    for i in range(1, num_features + 1):
        ...
        # Filter out tiny noise specks (e.g., 1-2 pixels)
        if area_km2 < 2.0:
            continue
        ...
        # Multi-threshold core counting (SCIT proxy)
        ...
        # 3. Calculate structural morphology (Major Axis Length & Eccentricity)
        y_indices, x_indices = np.where(cell_mask)
        if len(x_indices) > 0:
            # Central second moments (variances and covariance)
            mu20 = np.var(x_indices)
            mu02 = np.var(y_indices)
            mu11 = np.mean((x_indices - cx) * (y_indices - cy))
            
            # Analytical eigenvalues of the covariance matrix
            diff = (mu20 - mu02) / 2.0
            ...
            # Convert equivalent ellipse dimensions to scale metrics
...
def update_summaries(current_cells, previous_summaries, scene_motion, dt_min=None):
    ...
    for cell in current_cells:
        best_match = None
        best_dist = 15.0 # Max search radius in km (TITAN overlap threshold proxy)

        # 1. Match via Nearest Centroid
