...
# Range-dependent confidence taper (PDF Stage 1 §1c). The radar beam rises with
# range under the 4/3-Earth model, so distant echo is measured high aloft and
# may be virga or overshoot shallow surface rain. We taper confidence with
# distance and flag "echo aloft" beyond RANGE_TAPER_FAR_KM. NOTE: this uses
# distance-from-Budva as a proxy; the precise beam-height-from-the-source-radar
# version needs the DHMZ radar coordinates (a future refinement).
RANGE_TAPER_FULL_KM = 60.0
RANGE_TAPER_FAR_KM = 80.0
...
def interpret_source(source_id: str, location: dict, radii_km: list) -> dict:
    ...
    # --- Stage 1a: type-aware Z-R (activate the previously-dead pick_zr_scenario)
    # + explicit image dBZ quantization uncertainty (±2.5 dBZ ≈ ±50% in R) ---
    scene_max_dbz = None
    for r in rings:
        md = r.get("max_dbz")
        if md is not None and (scene_max_dbz is None or md > scene_max_dbz):
            scene_max_dbz = md
    zr_scenario = colormap.pick_zr_scenario(scene_max_dbz)

    # --- Stage 1b: range-dependent confidence taper + echo-aloft flag ---
    for r in rings:
        r["confidence"] = _range_confidence(r.get("radius_km"))
    echo_aloft = False
    ck = (approaching or {}).get("closest_rain_km")
    if (approaching and approaching.get("any_rain_within_radii")
            and ck is not None and ck > RANGE_TAPER_FAR_KM):
        echo_aloft = True
    ...
    return {
        ...
        "scenario": scenario,
        "zr_scenario": zr_scenario,
        "dbz_uncertainty": 2.5,
        "echo_aloft": echo_aloft,
    }
